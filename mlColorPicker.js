...
(function(jQuery) {
	var rgbRE = /^rgb\((\d+),\s*(\d+),\s*(\d+)\)jQuery/;
	var colorRE = /^[a-fA-F0-9]{6}|[a-fA-F0-9]{3}jQuery/;
	function mouseEnterColor(event){
		var rgbString = jQuery(event.target).css("background-color");
		...
		if(parts!=null){
			...
			jQuery("#mlSelectedColorText").val(hexString).keyup();
		}else{
			jQuery("#mlSelectedColorText").val(rgbString.substring(1)).keyup();
	...
	function colorChange(color){
		jQuery("#mlSelectedColorDiv").css("background-color", "#"+color);
	}
	function colorSave(){
		if (jQuery("#mlColorPicker").data("mlOnChange")) {
			color = jQuery("#mlSelectedColorText").val();
			if (color.match(colorRE)) {
				jQuery("#mlColorPicker").data("mlOnChange")(color);
				close()
			}
		}
	}
	...
	function close(){
		jQuery("#mlColorPicker").hide().removeData("mlOnChange");
	}
	...
	function createColorPicker(){
		...
		jQuery("body").append(('<div style="position:absolute;" id="mlColorPicker"><div id="mlSelectedColorDiv"></div><input id="mlSelectedColorText" type="text" maxlength="6"><div id="mlColors"></div></div>'))
		var colorHolder=jQuery("#mlColors")
		jQuery.each(colors, function(i, color){
			colorHolder.append('<div class="mlColor" style="background-color:' + color + '" />')
		});
		jQuery('.mlColor').bind("mouseenter", mouseEnterColor).click(mouseClickColor);
		jQuery("#mlSelectedColorText").keyup(textKeyUp);
		jQuery("#mlColorPicker").hide()
        jQuery(document).bind('click', function(e){
            jQuery("#mlColorPicker").hide();
        });
		jQuery('#mlColorPicker,#mlColorPicker *').click(function(e){e.stopPropagation()})
	}

	jQuery.fn.mlColorPicker = function(settings) {
		var config = {
			'onChange': function(value){}
		};

		if(jQuery("#mlColorPicker").length==0){
			createColorPicker()
		}
